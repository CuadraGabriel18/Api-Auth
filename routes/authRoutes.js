...
router.get('/google/callback',
  ...
  (req, res) => {
    ...
    res.redirect(`http://192.168.195.142:5500/login.html?token=${token}&username=${encodeURIComponent(username)}&email=${encodeURIComponent(email)}&role=${encodeURIComponent(role)}`);
  }
);
...
router.get('/google/register/callback', (req, res, next) => {
  ...
  passport.authenticate('google-register', { session: false }, async (err, user, info) => {
    if (err || !user) {
      return res.redirect('http://192.168.195.142:5500/login.html?error=Cuenta%20ya%20registrada');
    }
    ...
    res.redirect(`http://192.168.195.142:5500/login.html?token=${token}&username=${encodeURIComponent(username)}&email=${encodeURIComponent(email)}&role=${encodeURIComponent(user.role)}`);
  })(req, res, next);
});
